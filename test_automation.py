...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        # 4. Iterate Rows
        processed = 0
        start_row = int(args.start_row) if args.start_row else header_row + 1
        end_row = int(args.end_row) if args.end_row else int(ws.max_row or 0)
        for row_index in range(start_row, end_row + 1):
            ...
            try:
                ...
                if prev_output and actual_output == "" and prev_output != "":
                    print("Warning: Output did not update (still showing previous output). Attempting recovery...")
                    # Try direct API fallback first
                    recovered = ""
                    try:
                        recovered = direct_chat_translate(singlish_input).strip()
                        if recovered:
                            print(f"Recovered via direct API: {recovered!r}")
                            actual_output = recovered
                    except Exception as ex:
                        print(f"Direct API recovery failed: {ex}")

                    # If API didn't help, try reloading the page and re-reading output once
                    if not actual_output:
                        try:
                            print("Reloading page and retrying output read...")
                            page.reload(wait_until="domcontentloaded")
                            page.wait_for_timeout(max(0, int(args.retry_wait_ms)))
                            current = _read_output(is_chat, output_locator)
                            if current and current != prev_output:
                                actual_output = current
                                print("Recovered after reload.")
                        except Exception as ex:
                            print(f"Reload attempt failed: {ex}")

                    # If still no output, mark as UI Error and continue to next row
                    if not actual_output:
                        print("Recovery failed; marking row as UI Error and continuing.")
                        try:
                        ...
                        if args.save_every and int(args.save_every) > 0 and processed % int(args.save_every) == 0:
                            save_workbook_safely(wb, args.output)
                        continue
